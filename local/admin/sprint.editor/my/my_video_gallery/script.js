sprint_editor.registerBlock('my_video_gallery', function ($, $el, data) {
    data = $.extend({
        items: []
    }, data);
    ...
    var openedit = function (uid) {
        ...
        $el.find('.sp-edit').html(
            sprint_editor.renderTemplate('my_video_gallery-edit', itemsCollection[uid])
        ).show(250);
        ...
        $el.find('.sp-item-title').bindWithDelay('input', function () {
        ...
        $el.find('.sp-item-video').bindWithDelay('input', function () {
            ...
            if (val && !itemsCollection[uid].file && !itemsCollection[uid].file.ID) {
                $.ajax({
                    url: sprint_editor.getBlockWebPath('my_video_gallery') + '/download.php',
                    type: 'post',
                    data: {
                        url: val
                    },
        ...
        $btninput.fileupload({
            dropZone: $el,
            url: sprint_editor.getBlockWebPath('my_video_gallery') + '/upload.php',
            dataType: 'json',
    ...
    var renderitem = function (uid) {
        ...
        if ($item.length > 0) {
            $item.replaceWith(sprint_editor.renderTemplate('my_video_gallery-item', {
                item: item,
                uid: uid,
                active: 1
            }));
            ...
            $el.find('.sp-result').append(sprint_editor.renderTemplate('my_video_gallery-item', {
                item: item,
                uid: uid,
                active: 0
            }));
