sprint_editor.registerBlock('my_features', function ($, $el, data, settings) {
    settings = settings || {};
    ...
    this.collectData = function () {
        ...
        data.text = $el.find('.sp-text-value').val();

        data.items = [];

        $el.find('.my_features__sp-item').each(function () {
    ...
    this.afterRender = function () {
        ...
        $el.on('click', '.my_features__sp-item-del', function (evt) {
            var $image = $(evt.target).closest('.my_features__sp-item');
        ...
        $el.on('click', '.my_features__sp-item', function (evt) {
            var uid = $(this).data('uid');
            openedit(uid, $(this));
        });

        $el.find('.sp-text-value').trumbowyg({
    ...
    var openedit = function (uid, item=false) {
        ...
        var $btn = $el.find('.my_features__sp-file');
        var $btninput = $btn.find('input[type=file]');
        var $label = $btn.find('strong');
        var $text = $label.text();

        if(item) {
            item.find('.my_features__sp-item-title').bindWithDelay('input', function () {
        ...
        $btninput.fileupload({
            dropZone: $el,
            url: sprint_editor.getBlockWebPath('my_features') + '/upload.php',
            dataType: 'json',
        ...
        var $btnModal = $el.find('.my_features__sp-item-modal-file');
    ...
    var renderitem = function (uid) {
        ...
        if ($item.length > 0) {
            $item.replaceWith(sprint_editor.renderTemplate('my_features-item', {
                item: item,
                uid: uid,
                active: 1
            }));
        } else {
            $el.find('.my_features__sp-result').append(sprint_editor.renderTemplate('my_features-item', {
                item: item,
                uid: uid,
                active: 0
            }));
