sprint_editor.registerBlock('my_text_image', function ($, $el, data, settings) {
    ...
    this.collectData = function () {
        ...
        $el.find('.my_text_image__sp-item').each(function () {
    ...
    this.afterRender = function () {
        ...
        $el.on('click', '.my_text_image__sp-item-del', function (evt) {
            var $image = $(evt.target).closest('.my_text_image__sp-item');
        ...
        $el.on('click', '.my_text_image__sp-item', function (evt) {
            var uid = $(this).data('uid');
            openedit(uid, $(this));
        });
    };
    ...
    var openedit = function (uid, item=false) {
        ...
        var $btn = $el.find('.my_text_image__sp-file');
        var $btninput = $btn.find('input[type=file]');

        if(item) {
            item.find('.sp-title-image').bindWithDelay('input', function () {
                itemsCollection[uid].imageTitle = $(this).val();
            }, 500);
        ...
        $btninput.fileupload({
            dropZone: $el,
            url: sprint_editor.getBlockWebPath('my_text_image') + '/upload.php',
            dataType: 'json',
    ...
    var renderitem = function (uid) {
        ...
        if ($item.length > 0) {
            $item.replaceWith(sprint_editor.renderTemplate('my_text_image-item', {
                item: item,
                uid: uid,
                active: 1
            }));
        } else {
            $el.find('.my_text_image__sp-result').append(sprint_editor.renderTemplate('my_text_image-item', {
                item: item,
                uid: uid,
                active: 0
            }));
