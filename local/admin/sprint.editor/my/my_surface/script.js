sprint_editor.registerBlock('my_surface', function ($, $el, data, settings) {
    settings = settings || {};
    ...
    this.collectData = function () {
        ...
        $el.find('.my_surface__sp-item').each(function () {
    ...
    this.afterRender = function () {
        ...
        $el.on('click', '.my_surface__sp-item-del', function (evt) {
            var $image = $(evt.target).closest('.my_surface__sp-item');
        ...
        $el.on('click', '.my_surface__sp-item', function (evt) {
            var uid = $(this).data('uid');
            openedit(uid, $(this));
        });
    };
    ...
    var openedit = function (uid, item=false) {
        ...
        var $btn = $el.find('.my_surface__sp-file');
        var $btninput = $btn.find('input[type=file]');
        var $label = $btn.find('strong');
        var $text = $label.text();

        if(item) {
            item.find('.my_surface__sp-item-title').bindWithDelay('input', function () {
        ...
        $btninput.fileupload({
            dropZone: $el,
            url: sprint_editor.getBlockWebPath('my_surface') + '/upload.php',
            dataType: 'json',
        ...
        var $btnModal = $el.find('.my_surface__sp-item-modal-file');
    ...
    var renderitem = function (uid) {
        ...
        if ($item.length > 0) {
            $item.replaceWith(sprint_editor.renderTemplate('my_surface-item', {
                item: item,
                uid: uid,
                active: 1
            }));
        } else {
            $el.find('.my_surface__sp-result').append(sprint_editor.renderTemplate('my_surface-item', {
                item: item,
                uid: uid,
                active: 0
            }));
