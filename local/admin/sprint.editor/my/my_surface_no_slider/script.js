sprint_editor.registerBlock('my_surface_no_slider', function ($, $el, data, settings) {
    settings = settings || {};
    ...
    this.collectData = function () {
        ...
        $el.find('.my_surface_no_slider__sp-item').each(function () {
    ...
    this.afterRender = function () {
        ...
        $el.on('click', '.my_surface_no_slider__sp-item-del', function (evt) {
            var $image = $(evt.target).closest('.my_surface_no_slider__sp-item');
        ...
        $el.on('click', '.my_surface_no_slider__sp-item', function (evt) {
            var uid = $(this).data('uid');
            openedit(uid, $(this));
        });
    };
    ...
    var openedit = function (uid, item=false) {
        ...
        var $btn = $el.find('.my_surface_no_slider__sp-file');
        var $btninput = $btn.find('input[type=file]');
        var $label = $btn.find('strong');
        var $text = $label.text();

        if(item) {
            item.find('.my_surface_no_slider__sp-item-title').bindWithDelay('input', function () {
                itemsCollection[uid].surfaceName = $(this).val();
            }, 500);
        ...
        $btninput.fileupload({
            dropZone: $el,
            url: sprint_editor.getBlockWebPath('my_surface_no_slider') + '/upload.php',
            dataType: 'json',
        ...
        var $btnModal = $el.find('.my_surface_no_slider__sp-item-modal-file');
    ...
    var renderitem = function (uid) {
        ...
        if ($item.length > 0) {
            $item.replaceWith(sprint_editor.renderTemplate('my_surface_no_slider-item', {
                item: item,
                uid: uid,
                active: 1
            }));
        } else {
            $el.find('.my_surface_no_slider__sp-result').append(sprint_editor.renderTemplate('my_surface_no_slider-item', {
                item: item,
                uid: uid,
                active: 0
            }));
