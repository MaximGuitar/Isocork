sprint_editor.registerBlock('my_applicationation_type3', function ($, $el, data, settings) {
    ...
    this.collectData = function () {
        ...
        $el.find('.my_applicationation_type3__sp-item').each(function () {
    ...
    this.afterRender = function () {
        ...
        $el.on('click', '.my_applicationation_type3__sp-item-del', function (evt) {
            var $image = $(evt.target).closest('.my_applicationation_type3__sp-item');
        ...
        $el.on('click', '.my_applicationation_type3__sp-item', function (evt) {
            var uid = $(this).data('uid');
            openedit(uid, $(this));
        });
    ...
    var openedit = function (uid, item=false) {
        ...
        var $btn = $el.find('.my_applicationation_type3__sp-file');
        var $btninput = $btn.find('input[type=file]');

        if(item) {
            item.find('.my_applicationation_type3__sp-item-desc').bindWithDelay('input', function () {
        ...
        $btninput.fileupload({
            dropZone: $el,
            url: sprint_editor.getBlockWebPath('my_applicationation_type3') + '/upload.php',
            dataType: 'json',
    ...
    var renderitem = function (uid) {
        ...
        if ($item.length > 0) {
            $item.replaceWith(sprint_editor.renderTemplate('my_applicationation_type3-item', {
                item: item,
                uid: uid,
                active: 1
            }));
        } else {
            $el.find('.my_applicationation_type3__sp-result').append(sprint_editor.renderTemplate('my_applicationation_type3-item', {
                item: item,
                uid: uid,
                active: 0
            }));
