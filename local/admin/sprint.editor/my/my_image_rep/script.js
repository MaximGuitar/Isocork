sprint_editor.registerBlock('my_image_rep', function ($, $el, data, settings) {
    ...
    var openedit = function (uid, item=false) {
        ...
        $btninputIcon.fileupload({
            url: sprint_editor.getBlockWebPath('my_image_rep') + '/upload.php',
            dataType: 'json',
            done: function (e, result) {
    ...
    var renderitem = function (uid) {
        ...
        if ($item.length > 0) {
            $item.replaceWith(sprint_editor.renderTemplate('my_image_rep-item', {
                item: item,
                uid: uid,
                active: 1
            }));
        } else {
            $el.find('.my_applicationation__sp-result').append(sprint_editor.renderTemplate('my_image_rep-item', {
                item: item,
                uid: uid,
                active: 0
            }));
