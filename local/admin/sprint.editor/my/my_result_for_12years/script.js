sprint_editor.registerBlock('my_result_for_12years', function ($, $el, data, settings) {
    ...
    this.collectData = function () {
        ...
        $el.find('.my_result_for_12years__sp-item').each(function () {
    ...
    this.afterRender = function () {
        ...
        $btninput.fileupload({
            dropZone: $el,
            url: sprint_editor.getBlockWebPath('my_result_for_12years') + '/upload.php',
            dataType: 'json',
        ...
        $el.on('click', '.my_result_for_12years__sp-item-del', function (evt) {
            var $image = $(evt.target).closest('.my_result_for_12years__sp-item');
        ...
        $el.on('click', '.my_result_for_12years__sp-item', function (evt) {
            var uid = $(this).data('uid');
            openedit(uid, $(this));
        });
    };
    ...
    var openedit = function (uid, item=false) {
        ...
        var $btn = $el.find('.my_result_for_12years__sp-file');
        var $btninput = $btn.find('input[type=file]');

        if(item) {
            item.find('.my_result_for_12years__sp-title').bindWithDelay('input', function () {
                itemsCollection[uid].title = $(this).val();
            }, 500);
            item.find('.my_result_for_12years__sp-button_text').bindWithDelay('input', function () {
                itemsCollection[uid].button_text = $(this).val();
            }, 500);
            item.find('.my_result_for_12years__sp-button_url').bindWithDelay('input', function () {
    ...
    var renderitem = function (uid) {
        ...
        if ($item.length > 0) {
            $item.replaceWith(sprint_editor.renderTemplate('my_result_for_12years-item', {
                item: item,
                uid: uid,
                active: 1
            }));
        } else {
            $el.find('.my_result_for_12years__sp-result').append(sprint_editor.renderTemplate('my_result_for_12years-item', {
                item: item,
                uid: uid,
                active: 0
            }));
    ...
    var renderfiles = function () {
        $el.find('.sp-result').html(
            sprint_editor.renderTemplate('my_result_for_12years-image', data)
        );
    };
