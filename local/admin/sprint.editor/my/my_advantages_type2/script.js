sprint_editor.registerBlock('my_advantages_type2', function ($, $el, data, settings) {
    ...
    this.collectData = function () {
        data.title = $el.find('.my_advantages_type2__sp-title').val();
        data.titleDescription = $el.find('.sp-title').val();

        data.items = [];

        $el.find('.my_advantages_type2__sp-item').each(function () {
    ...
    this.afterRender = function () {
        ...
        $btninput.fileupload({
            dropZone: $el,
            url: sprint_editor.getBlockWebPath('my_advantages_type2') + '/upload.php',
            dataType: 'json',
        ...
        $el.on('click', '.my_advantages_type2__sp-item-del', function (evt) {
            var $image = $(evt.target).closest('.my_advantages_type2__sp-item');;
        ...
        $el.on('click', '.my_advantages_type2__sp-item', function () {
            var uid = $(this).data('uid');
            openedit(uid, $(this));
        });
        
        var removeIntent = false;
        // $el.find('.my_advantages_type2__sp-result').sortable({
        //     items: ".my_advantages_type2__sp-item",
    ...
    var renderfiles = function () {
        $el.find('.sp-result').html(
            sprint_editor.renderTemplate('my_advantages_type2-image', data)
        );
    };
    ...
    var openedit = function (uid, item=false) {
        ...
        var $btn = $el.find('.my_advantages_type2__sp-file');
        var $btninput = $btn.find('input[type=file]');

        if(item) {
            item.find('.my_advantages_type2__sp-item-desc').bindWithDelay('input', function () {
                itemsCollection[uid].desc = $(this).val();
            }, 500);

            item.find('.my_advantages_type2__sp-item-title').bindWithDelay('input', function () {
            ...
            area.addEventListener("DOMSubtreeModified", function() {
                ...
                console.log(itemsCollection[uid].modalText);
            });
        }
        ...
        var $btnModal = $el.find('.my_advantages_type2__sp-item-modal-file');
    ...
    var renderitem = function (uid) {
        ...
        if ($item.length > 0) {
            $item.replaceWith(sprint_editor.renderTemplate('my_advantages_type2-item', {
                item: item,
                uid: uid,
                active: 1
            }));
            ...
            $el.find('.my_advantages_type2__sp-result').append(sprint_editor.renderTemplate('my_advantages_type2-item', {
                item: item,
                uid: uid,
                active: 0
            }));
