sprint_editor.registerBlock('my_sostav_probkovoy_smesi', function ($, $el, data, settings) {
    ...
    this.collectData = function () {
        ...
        $el.find('.my_sostav_probkovoy_smesi__sp-item').each(function () {
    ...
    this.afterRender = function () {
        ...
        $el.on('click', '.my_sostav_probkovoy_smesi__sp-item-del', function (evt) {
            var $image = $(evt.target).closest('.my_sostav_probkovoy_smesi__sp-item');
        ...
        $el.on('click', '.my_sostav_probkovoy_smesi__sp-item', function (evt) {
            var uid = $(this).data('uid');
            openedit(uid, $(this));
        });
    };
    ...
    var openedit = function (uid, item=false) {
        ...
        var $btn = $el.find('.my_sostav_probkovoy_smesi__sp-file');
        var $btninput = $btn.find('input[type=file]');

        if(item) {
            item.find('.my_sostav_probkovoy_smesi__sp-title').bindWithDelay('input', function () {
                itemsCollection[uid].title = $(this).val();
            }, 500);
            item.find('.my_sostav_probkovoy_smesi__sp-button_text').bindWithDelay('input', function () {
                itemsCollection[uid].button_text = $(this).val();
            }, 500);
            item.find('.my_sostav_probkovoy_smesi__sp-button_url').bindWithDelay('input', function () {
        ...
        $btninput.fileupload({
            dropZone: $el,
            url: sprint_editor.getBlockWebPath('my_sostav_probkovoy_smesi') + '/upload.php',
            dataType: 'json',
    ...
    var renderitem = function (uid) {
        ...
        if ($item.length > 0) {
            $item.replaceWith(sprint_editor.renderTemplate('my_sostav_probkovoy_smesi-item', {
                item: item,
                uid: uid,
                active: 1
            }));
        } else {
            $el.find('.my_sostav_probkovoy_smesi__sp-result').append(sprint_editor.renderTemplate('my_sostav_probkovoy_smesi-item', {
                item: item,
                uid: uid,
                active: 0
            }));
